...
if not threads:
    ...
    for chat in threads:
      ...
      if st.sidebar.button(
         chat["title"],
         key=f"side-thread-{chat['id']}",
         use_container_width=True,
      ):

         selected_thread = chat["id"]
...
for chat in st.session_state["chat_threads"]:
    if chat["id"] == selected_thread:
        selected_chat = chat
        break

if selected_chat:
    st.session_state["chat_threads"].remove(selected_chat)
    st.session_state["chat_threads"].insert(0, selected_chat)
